...
- 📋 **复制配置** - 手动复制配置内容
- 🔄 **连续生成** - 在同一会话中生成多个配置,peer.txt 会自动累积所有配置(一行一个)

**重要提示:**
- 所有配置文件(wg0.conf 和 peer.txt)都需要用户手动选择保存位置,不会自动保存到应用目录,这样可以避免生产环境中的只读文件系统错误
- peer.txt 会累积本次会话生成的所有 Peer 配置,方便批量导入到爱快路由器
- 点击"清空累积配置"可以重新开始累积
